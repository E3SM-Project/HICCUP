...
###SBATCH --job-name=hiccup
###SBATCH --output=logs_slurm/slurm-%x-%j.out
###SBATCH --mail-user=<name>@<lab>.gov
###SBATCH --mail-type=END,FAIL
#SBATCH --mem=0

# To run this batch script, use the command below to set the output grid from the command line:
# NE=1024; VGRID=L128; sbatch --job-name=hiccup_ne$NE --output=logs_slurm/slurm-%x-%j.out --export=NE=$NE,VGRID=$VGRID ./run_hiccup_batch.rhea.sh
...
python -u ./create_initial_condition.py --hgrid=ne${NE}np4 --vgrid=$VGRID
