...
#-------------------------------------------------------------------------------
def adjust_surface_state(plev, ncol, temperature,     \
                         pressure_mid, pressure_int,  \
                         phis_old, ps_old, ts_old,    \
                         phis_new, ps_new, ts_new)

  lapse   = 0.0065       # assumed lapse rate            ~ -6.5 K/km
  ...
  T_ref1    = 290.5       # reference temperature
  T_ref2    = 255.0       # reference temperature
  
  phis_threshold = 0.001  # threshold for determining whether to calculate new pressure
  dz_min = 150.           # min distance of first level above sfc for Tbot and Pbot
  ...
  for i in range(1,ncol) :
    ...
    if(np.abs(del_phis) <= phis_threshold) :
      ...
      if (Tstar <= T_ref1) and (T0 > T_ref1) :
        # inhibit low pressure under 
        # elevated hot terrain
        alpha = rdair/phis_old[i]*( T_ref1 - Tstar )        # pg 9 eq 14.1
      elif (Tstar > T_ref1) and (T0 > T_ref1) :
        # inhibit low pressure under 
        # elevated hot terrain
        Tstar = ( T_ref1 + Tstar )*0.5                      # pg 9 eq 14.2
      if (Tstar < T_ref2) :
        # inhibit unduly high pressure 
        # below elevated cold terrain
        Tstar = ( T_ref2 + Tstar )*0.5                      # pg 9 eq 14.3

      # Calculate new surface pressure
      beta = phis_old[i]/(rair*Tstar)
      temp = beta*(1. - 0.5*alph*beta + (1./3.)*(alph*beta)**2. )
