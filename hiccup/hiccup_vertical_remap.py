...
def _input_pressure_spec(ds, lev_name):
  ...
  if hybrid and 'P0' in variables:
    return 'hybrid', (ds['hyam'], ds['hybm'], ds['P0'])

  if hybrid and 'lnsp' in variables:
    # IFS layout: hyam is already in Pa, so the P0 multiplier is unity
    return 'hybrid', (ds['hyam'], ds['hybm'], 1.0)

  if lev_name in ds.coords or lev_name in ds.variables:
    return 'plev', ds[lev_name].astype('float64')
