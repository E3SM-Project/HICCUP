# Alternative E3SM Initial Condition Remapping Workflow

This tool is an alternative to HICCUP that was developed by Wuyin Lin (Brookhaven National Laboratory). It was originally written in IDL, but then rewritten in python by Qi Tang (Lawrence Livermore National Laboratory).

This tool duplicates HICCUP functionality and is not the preferred workflow for E3SM. This is because it uses the ESMF mapping tool instead of the preferred TempestRemap, it is not adapted to handle EAMxx conventions, and it was not built to scale well for large grids. However, this is a simpler alternative to HICCUP and has been used in various E3SM simulation campaigns. So for these reasons we have put it here for reference/provenance.

The purpose of this tool is to remap E3SM atmospheric initial conditions (output from the model) from one grid to another with proper surface pressure adjustment for topography differences to minimize initialization shock.

Users may need to edit the following input variables in the driver `remap_atmIC_m2m.sh` (m2m stands for model-to-model).
